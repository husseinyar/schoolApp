...
import { MessageSquare, Send, Loader2, User, ChevronLeft, AlertCircle, Plus, Search, ChevronRight } from "lucide-react";
...
export default function AdminMessagesPage() {
  ...
  async function selectConversation(id: string) {
    try {
      ...
      setIsNewChatOpen(false); // Close new chat dialog if open
    } catch (err) {
      console.error(err);
    }
  }
  ...
  async function searchUsers(q: string) {
    ...
    setSearching(true);
    try {
      const res = await fetch(`/api/users?search=${q}&limit=10`);
      const data = await res.json();
      if (data.success) {
        // Filter out current user
        setUsers(data.data.users.filter((u: any) => u.id !== session?.user?.id));
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  async function startNewChat(recipientId: string) {
    setSending(true);
    try {
      const res = await fetch("/api/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipientId, content: "Initial message" }),
      });
      const data = await res.json();
      if (data.message) {
        await selectConversation(data.message.conversationId);
        loadConversations();
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Admin Inbox</h1>
          <p className="text-slate-400 text-sm mt-1">Manage communications with parents and drivers.</p>
        </div>
      ...
      <div className="h-[calc(100vh-16rem)] flex bg-slate-900/40 backdrop-blur border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className={`w-full md:w-80 border-r border-slate-800 flex flex-col ${activeConv ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-5 border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2 text-indigo-400">
              ...
              <h2 className="font-bold text-white">Active Chats</h2>
            </div>
          </div>
            ...
            {conversations.length === 0 ? (
              ...
              conversations.map((conv) => {
                ...
                return (
                  ...
                  >
                    <div className="w-12 h-12 rounded-2xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 flex-shrink-0 relative">
                      <User className="w-6 h-6" />
                      <span className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-slate-900 ${other?.role === 'PARENT' ? 'bg-teal-500' : 'bg-amber-500'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-0.5">
                        <p className="text-white font-bold truncate text-sm">{other?.name || "User"}</p>
                        <span className="text-[10px] text-slate-500 font-medium">
                          {new Date(conv.lastMessageAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 font-bold uppercase tracking-wider">{other?.role}</span>
                        <p className="text-slate-500 text-xs truncate flex-1 leading-none">
                          {lastMsg?.body || "New conversation started"}
                        </p>
                      </div>
                    </div>
                  </button>
          ...
          {!activeConv ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-4 p-8">
              ...
              <div className="text-center">
                ...
                <p className="text-sm mt-1 max-w-[250px]">Choose a chat from the sidebar or start a new one to communicate.</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="p-4 border-b border-slate-800 bg-slate-900/20 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-white font-bold text-sm">
                    {activeConv.participants.find(p => p.id !== session?.user?.id)?.name || "User"}
                  </p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-indigo-400 text-[10px] font-bold uppercase tracking-widest">
                       {activeConv.participants.find(p => p.id !== session?.user?.id)?.role}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2 custom-scrollbar">
              {searching ? (
                ...
                <p className="text-center py-8 text-slate-500 text-sm">{searchQuery.length < 2 ? 'Type at least 2 characters' : 'No users found'}</p>
              ) : (
                users.map((u) => (
                  <button
                    key={u.id}
                    onClick={() => startNewChat(u.id)}
                    className="w-full p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 flex items-center justify-between transition group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest">{u.role} · {u.email}</p>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-slate-700 group-hover:text-indigo-400 transition" />
                  </button>
                ))
              )}
            </div>
